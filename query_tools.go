...
func QuerySelect(ctx context.Context, req *mcp.CallToolRequest, input QuerySelectInput) (*mcp.CallToolResult, any, error) {
...
func QueryInsert(ctx context.Context, req *mcp.CallToolRequest, input QueryInsertInput) (*mcp.CallToolResult, any, error) {
	if readOnly {
		return nil, nil, fmt.Errorf("database is in read-only mode")
	}
	...
	if err != nil {
		return nil, nil, fmt.Errorf("insert failed: %w", err)
	}
...
func QueryUpdate(ctx context.Context, req *mcp.CallToolRequest, input QueryUpdateInput) (*mcp.CallToolResult, any, error) {
	if readOnly {
		return nil, nil, fmt.Errorf("database is in read-only mode")
	}
	...
	if len(input.Where) == 0 {
		return nil, nil, fmt.Errorf("WHERE clause is required for UPDATE")
	}
	...
	if err := db.QueryRowContext(ctx, countSQL, countArgs...).Scan(&rowCount); err != nil {
		return nil, nil, fmt.Errorf("failed to check row count: %w", err)
	}

	if rowCount > maxUpdateLimit {
		return nil, nil, fmt.Errorf("UPDATE would affect %d row(s), which exceeds the maximum limit of %d. Please refine your WHERE clause to target fewer rows", rowCount, maxUpdateLimit)
	}
...
func QueryDelete(ctx context.Context, req *mcp.CallToolRequest, input QueryDeleteInput) (*mcp.CallToolResult, any, error) {
	if readOnly {
		return nil, nil, fmt.Errorf("database is in read-only mode")
	}
	...
	if len(input.Where) == 0 {
		return nil, nil, fmt.Errorf("WHERE clause is required for DELETE")
	}
	...
	if rowCount > maxDeleteLimit {
		return nil, nil, fmt.Errorf("DELETE would affect %d row(s), which exceeds the maximum limit of %d. Please refine your WHERE clause to target fewer rows", rowCount, maxDeleteLimit)
	}
...
func QueryRaw(ctx context.Context, req *mcp.CallToolRequest, input QueryRawInput) (*mcp.CallToolResult, any, error) {
	if !allowRawQuery {
		return nil, nil, fmt.Errorf("raw SQL queries are blocked. Set ALLOW_RAW_QUERY=true to enable this dangerous feature")
	}
	...
	if dbType == "mysql" {
		...
		if err != nil {
			return nil, nil, fmt.Errorf("failed to switch to database %s: %w", input.Database, err)
		}
	}
